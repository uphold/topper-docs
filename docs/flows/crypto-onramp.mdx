...
:::note
At least one of `source.amount` or `target.amount` _must_ be provided, but not both. Assets for `source` and `target` that are available for this flow can be found using our [REST API](../rest-api.md), via the [assets endpoint](https://api.topperpay.com/assets/crypto-onramp).
:::
