...
Webhooks can be configured through the [Enterprise Portal](https://portal.enterprise.uphold.com/). Navigate to the Topper product, select the widget, and open the Webhooks tab. From there, you can create webhooks and choose the events you wish to receive for each one.

For every webhook, a corresponding public key is generated and made available to you. This key can be used to securely verify that incoming webhook requests were sent by Topper.

Account managers are available to assist with the configuration process if needed.
